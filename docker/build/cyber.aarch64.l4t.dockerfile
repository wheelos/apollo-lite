...
RUN bash -eux /opt/apollo/installers/install_minimal_environment.sh ${GEOLOC} \
    && bash -eux /opt/apollo/installers/install_bazel.sh \
    && bash -eux /opt/apollo/installers/install_cmake.sh ${INSTALL_MODE} \
    && bash -eux /opt/apollo/installers/install_llvm_clang.sh \
    && bash -eux /opt/apollo/installers/install_cyber_deps.sh ${INSTALL_MODE} \
    && bash -eux /opt/apollo/installers/install_visualizer_deps.sh ${INSTALL_MODE} \
    && bash -eux /opt/apollo/installers/post_install.sh cyber
