...
TORCHVISION_VERSION="0.21.0" # Note: Version must be compatible with Pytorch
TORCHAUDIO_VERSION="2.6.0"  # Note: Version must be compatible with Pytorch
...
function install_libtorch_cpp() {
  ...
  # aarch64: Build libtorch from source
    ...
    # git clone --depth 1 \
    #   --single-branch --branch "v${PYTORCH_VERSION}" \
    #   --recurse-submodules \
    #   --shallow-submodules \
    ...
    local PYTORCH_SITE="/usr/local/lib/python3.10/dist-packages/torch"
