...
bool RoutingComponent::Proc(const std::shared_ptr<RoutingRequest>& request) {
  ...
  if (!routing_.Process(request, response.get())) {
    // TODO(zero): In order to achieve autonomous driving based on trajectory
    // points, when there is no road, path planning can be done directly based
    // on key points.
    // Need to refactor to be compatible with both modes.
