...
void convert_model_params(const smoke::ModelParam &model_param,
                          smoke::StModelParam *st_model_param) {
...
bool SmokeObstacleDetector::Init(const StageConfig &stage_config) {
  ...
  auto smoke_obstacle_detection_config_ = stage_config.camera_detector_config();
...
bool SmokeObstacleDetector::Preprocessor(
...
bool SmokeObstacleDetector::Detect(const ObstacleDetectorOptions &options,
  ...
  smoke::StModelParam st_model_param;
  convert_model_params(smoke_param_.model_param(), &st_model_param);
  get_smoke_objects_cpu(
      smoke_blobs_, types_, st_model_param, light_vis_conf_threshold_,
