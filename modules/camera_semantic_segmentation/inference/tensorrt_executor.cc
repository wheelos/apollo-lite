...
class TensorRtSegFormerExecutor::Impl {
 public:
  struct Buffer {
    std::string name;
  ...
  bool Init(const SegFormerModelOptions& options) {
    ...
    input_.shape = {1, static_cast<int64_t>(kSegFormerInputChannels),
                    static_cast<int64_t>(options.preprocess.height),
                    static_cast<int64_t>(options.preprocess.width)};
    if (options.output_layout == "NCHW") {
      output_.shape = {1, static_cast<int64_t>(options.num_classes),
                       static_cast<int64_t>(options.preprocess.height),
                       static_cast<int64_t>(options.preprocess.width)};
    } else {
      output_.shape = {1, static_cast<int64_t>(options.preprocess.height),
                       static_cast<int64_t>(options.preprocess.width),
    ...
    if (!file.is_open()) {
      AERROR << "Unable to open SegFormer TensorRT engine: "
             << options.engine_path;
      return false;
    }
    const std::streamsize file_size = file.tellg();
    if (file_size <= 0) {
      AERROR << "SegFormer TensorRT engine is empty: " << options.engine_path;
      return false;
    }
    std::vector<char> engine_bytes(static_cast<std::size_t>(file_size));
    file.seekg(0, std::ios::beg);
    if (!file.read(engine_bytes.data(), file_size)) {
      AERROR << "Unable to read SegFormer TensorRT engine: "
             << options.engine_path;
      return false;
    }
    ...
    if (engine_ == nullptr || !ConfigureBuffers()) {
      AERROR << "TensorRT engine does not match the SegFormer I/O contract";
      return false;
    }
  ...
  bool Run(const std::vector<float>& input, SegFormerTensor* output) {
...
TensorRtSegFormerExecutor::TensorRtSegFormerExecutor() : impl_(new Impl) {}

TensorRtSegFormerExecutor::~TensorRtSegFormerExecutor() = default;

bool TensorRtSegFormerExecutor::Init(const SegFormerModelOptions& options) {
  return impl_->Init(options);
}

bool TensorRtSegFormerExecutor::Run(const std::vector<float>& input,
                                    SegFormerTensor* output) {
