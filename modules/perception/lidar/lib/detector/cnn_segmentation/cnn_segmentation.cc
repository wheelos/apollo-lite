...
bool CNNSegmentation::Init(const LidarDetectorInitOptions& options) {
  ...
  StFeatureParam st_feature_param;
  st_feature_param.point_cloud_range = feature_param.point_cloud_range();
  st_feature_param.width = feature_param.width();
  st_feature_param.height = feature_param.height();
  st_feature_param.min_height = feature_param.min_height();
  st_feature_param.max_height = feature_param.max_height();
  st_feature_param.use_intensity_feature =
      feature_param.use_intensity_feature();
  st_feature_param.use_constant_feature = feature_param.use_constant_feature();
  ACHECK(feature_generator_->Init(st_feature_param, feature_blob_.get()))
...
bool CNNSegmentation::Init(const StageConfig& stage_config) {
  ...
  inference_.reset(inference::CreateInferenceByName(
      cnnseg_param_.model_type(), cnnseg_config_.proto_file(),
      cnnseg_config_.weight_file(), output_names, input_names));
  CHECK_NOTNULL(inference_.get());
