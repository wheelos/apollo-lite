...
#include "modules/perception/lidar/lib/detector/graph_segmentation/common/object_split.h"
...
void SplitObject(const base::ObjectPtr& obj,
  ...
  Eigen::Vector2f min_xy(std::numeric_limits<float>::max(),
                         std::numeric_limits<float>::max());
  Eigen::Vector2f max_xy(-std::numeric_limits<float>::max(),
                         -std::numeric_limits<float>::max());
  ...
  for (auto& obj : *split_objs) {
    if (obj->lidar_supplement.cloud.size() > 3) {
      obj->lidar_supplement.num_points_in_roi =
...
bool LineFit2D(const base::PointFCloud& cloud, Eigen::Vector3f* params) {
  ...
  Eigen::Vector2f normal = std::abs(values(0)) < std::abs(values(1))
