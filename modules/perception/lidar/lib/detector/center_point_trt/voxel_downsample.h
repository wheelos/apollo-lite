...
struct compareX {
  __host__ __device__ bool operator()(apollo::perception::base::PointF lp,
                                      apollo::perception::base::PointF rp) {
    return lp.x < rp.x;
  }
};
...
class VoxelDownSampleCuda {
 public:
  VoxelDownSampleCuda() = default;
  ~VoxelDownSampleCuda() = default;

  cudaError_t cudaCalculateGridParams(
      apollo::perception::base::PointF* d_point_cloud, int number_of_points,
      float resolution_X, float resolution_Y, float resolution_Z,
      gridParameters& out_rgd_params);

  cudaError_t cudaCalculateGrid(int threads,
                                apollo::perception::base::PointF* d_point_cloud,
                                hashElement* d_hashTable, int number_of_points,
                                gridParameters rgd_params, int* count);

  cudaError_t cudaDownSample(int threads,
                             apollo::perception::base::PointF* d_point_cloud,
                             bool* d_markers, hashElement* d_hashTable,
                             voxel* d_voxel, int number_of_points,
                             gridParameters rgd_params,
                             bool use_centroid_downsample, int* count);
