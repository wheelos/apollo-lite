...
#include "modules/perception/pipeline/proto/stage/center_point_trt_detection_config.pb.h"
...
struct ResultsOutput {
  ...
  inline void AddPointId(int point_id) { point_ids.push_back(point_id); }
  inline void clear() {
    x = 0.0;
    y = 0.0;
    z = 0.0;
...
class CenterPointTRT : public BaseLidarDetector {
 public:
  CenterPointTRT();
  ~CenterPointTRT() override = default;
  ...
  std::string Name() const override { return "CenterPointTRT"; }
  ...
  void GenerateBackboneFeature(const base::Blob<float>* pillar_feature_blob) {
    if (use_gpu_generate_feature_) {
  ...
  void GetObjectsAndAssignPoints() {
    ...
    if (use_cpu_assign_points_) {
      ...
      AERROR << "Error: Not Support AssignPoints is False when GetObjects is "
                "not False";
      return;
    }
  }
 ...
 private:
  void LoadParams(const CenterPointTRTDetectionConfig& cpdet_config);

  bool InitModel(const CenterPointTRTDetectionConfig::ModelParam& model_param);
  ...
  template <typename T>
  void HOST_SAVE(const T* array, int row, int col, std::string filename,
                 bool is_canvus) {
  ...
  template <typename T>
  void DEVICE_SAVE(const T* array, int row, int col, std::string filename,
                   bool is_canvus = false) {
 ...
 private:
  // stage config
  CenterPointTRTDetectionConfig cpdet_config_;

  std::shared_ptr<inference::MultiBatchInference> pfn_inference_;
  std::shared_ptr<inference::MultiBatchInference> backbone_inference_;
  ...
  float score_thresh_ = 0.1f;
  float nms_overlap_thresh_ = 0.01f;
  int min_pts_num_fg_ = 5;
  int max_candidate_num_ = 1;
  ...
  std::unique_ptr<GpuDownSample> down_sample_;
