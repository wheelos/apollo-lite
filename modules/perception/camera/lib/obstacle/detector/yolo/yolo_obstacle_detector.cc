...
#include "modules/perception/camera/common/temporary_transition_types.h"
...
bool YoloObstacleDetector::Init(const StageConfig &stage_config) {
  ...
  auto yolo_obstacle_detector_config_ = stage_config.camera_detector_config();

  gpu_id_ = yolo_obstacle_detector_config_.gpu_id();
  BASE_CUDA_CHECK(cudaSetDevice(gpu_id_));
  BASE_CUDA_CHECK(cudaStreamCreate(&stream_));

  std::string camera_name = yolo_obstacle_detector_config_.camera_name();
...
bool YoloObstacleDetector::Process(DataFrame *data_frame) {
  ...
  get_objects_cpu(yolo_blobs_, stream_, types_, nms_, st_model_param,
