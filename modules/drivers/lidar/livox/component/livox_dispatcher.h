...
class LivoxDispatcher {
 public:
  ...
  using PointCloudCallbackType =
      std::function<void(uint32_t handle, const uint8_t dev_type,
                         LivoxLidarEthernetPacket* data, void* client_data)>;

  bool GetHandleFromIP(const std::string& ip, uint32_t& handle);

  void RegisterHandleDispatchCallback(uint32_t handle,
...
static void GlobalPointCloudCallback(uint32_t handle, const uint8_t dev_type,
                                     LivoxLidarEthernetPacket* data,
                                     void* client_data) {
  auto* dispatcher_pointer = reinterpret_cast<LivoxDispatcher*>(client_data);
  dispatcher_pointer->LivoxPointCloudCallback(handle, dev_type, data,
                                              client_data);
}
