...
void LslidarDriver::Init() {
  ...
  if (!config_.pcap_path().empty()) {
    ...
    if (config_.model() == LSLIDAR_C32_V4 ||
      ...
      positioning_input_.reset(new InputSocket(config_.difop_port(),
                                               config_.device_ip(),
                                               1206));  // 设备包
    } else {
      // open Lslidar input device
...
int LslidarDriver::PollStandard(
  ...
  if (config_.model() == LSLIDAR16P || config_.model() == LSLIDAR32P ||
    ...
    // AERROR << "scan->firing_pkts_size(): " << scan->firing_pkts_size() << "
    // config_.npackets(): " <<config_.npackets(); if(scan->firing_pkts_size() >
    // config_.npackets()) AERROR << "-------------- scan->firing_pkts_size(): "
    // << scan->firing_pkts_size();
    while (scan->firing_pkts_size() < config_.npackets() &&
           !is_found_frame_header) {
      // while (!is_found_frame_header) {
      // if(scan->firing_pkts_size() > 1400) {
      //     AERROR << "-------------- scan->firing_pkts_size(): " <<
