...
void LslidarComponent::ScanQueuePollProcess() {
  ...
  while (!apollo::cyber::IsShutdown()) {
    if (scan_queue_.popWait(scan_frame, 1000)) {
...
void LslidarComponent::HandleScanFrame(
  ...
  double last_point_time =
      point_cloud_out->point(point_cloud_out->point_size() - 1).timestamp();

  point_cloud_out->mutable_header()->set_timestamp_sec(
      cyber::Time().Now().ToSecond());  // 系统时间
  point_cloud_out->mutable_header()->set_lidar_timestamp(
      last_point_time);  // 最后一个点的时间
  point_cloud_out->set_measurement_time(last_point_time *
                                        1e-9);  // 最后一个点的时间
  ...
  // AERROR << " Last Point Time: " <<
  // point_cloud_out->point(point_cloud_out->point().size() - 1).timestamp();
  // AERROR << "";
