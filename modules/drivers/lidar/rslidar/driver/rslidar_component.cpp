...
bool RslidarComponent::Init() {
  ...
  driver_ptr_ =
      std::make_shared<::robosense::lidar::LidarDriver<PointCloudMsg>>();
  ...
  if (conf_.config_base().source_type() ==
    ...
    driver_ptr_->regPacketCallback(std::bind(
        &RslidarComponent::RsPacketCallback, this, std::placeholders::_1));
  }

  driver_ptr_->regPointCloudCallback(
      std::bind(&RslidarComponent::RsCloudAllocateCallback, this),
      std::bind(&RslidarComponent::RsCloudPutCallback, this,
                std::placeholders::_1));
  driver_ptr_->regExceptionCallback([](const ::robosense::lidar::Error& code) {
...
void RslidarComponent::ReadScanCallback(
  ...
  scan_packet->buf_.assign(scan_message->data().begin(),
                           scan_message->data().end());
