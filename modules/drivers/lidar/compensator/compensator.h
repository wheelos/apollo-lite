...
class Compensator {
 public:
  explicit Compensator(const LidarCompensatorConfig& config)
      : config_(config) {}
  virtual ~Compensator() {}

  bool MotionCompensation(
      const std::shared_ptr<const apollo::drivers::PointCloud>& msg,
      std::shared_ptr<apollo::drivers::PointCloud> msg_compensated);
