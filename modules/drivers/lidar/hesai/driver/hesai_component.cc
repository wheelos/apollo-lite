...
bool HesaiComponent::Init() {
  ...
  param.decoder_param.pcap_play_in_loop = true;
  ...
  // Note: lidar_type is no longer set in DriverParam in SDK 2.0
  // The lidar type is auto-detected by the SDK

  // Use explicit std::function type to resolve callback overload ambiguity
  std::function<void(const LidarDecodedFrame<LidarPointXYZIRT>&)>
      point_cloud_cb = std::bind(&HesaiComponent::SendPointCloud, this,
                                 std::placeholders::_1);
  driver_ptr_->RegRecvCallback(point_cloud_cb);
  ...
  if (conf_.config_base().source_type() ==
    ...
    // Use explicit std::function type to resolve callback overload ambiguity
    std::function<void(const UdpFrame_t&, double)> packet_cb =
        std::bind(&HesaiComponent::SendPacket, this, std::placeholders::_1,
                  std::placeholders::_2);
    driver_ptr_->RegRecvCallback(packet_cb);
  }
...
void HesaiComponent::ReadScanCallback(
...
void HesaiComponent::SendPointCloud(
...
#pragma omp parallel for schedule(static) num_threads(convert_threads_num_)
  for (int i = 0; i < point_size; ++i) {
    ...
    cloud_message->mutable_point(i)->set_timestamp(
        GetNanosecondTimestampFromSecondTimestamp(msg.points[i].timestamp));
...
void HesaiComponent::SendPacket(const hesai::lidar::UdpFrame_t& hesai_raw_msg,
                                double timestamp) {
  std::shared_ptr<HesaiUdpFrame> udp_frame = std::make_shared<HesaiUdpFrame>();

  for (size_t i = 0; i < hesai_raw_msg.size(); ++i) {
