...
默认配置文件是AT128P的配置文件，路径`/apollo/modules/drivers/lidar/hesai/conf/hesai.pb.txt`

| 禾赛SDK会自动检查设备型号，所以不再需要更改`lidar_type`
...
4. 更改`source_type`，1对应一般lidar输入，2对应pcap输入，3对应scan数据输入。使用scan数据输入需要先将config_base中的source_type调成RAW_PACKET，再将配置文件里source_type调成3
...
```sh
mainboard -d /apollo/modules/drivers/lidar/hesai/dag/hesai.dag
```
