...
bool SeyondDriver::setup_lidar() {
  ...
  std::string client_set_key = "";
  client_set_key = "LidarClient_Communication/get_conn_timeout_sec";
  ret =
      inno_lidar_set_config_name_value(handle_, client_set_key.c_str(), "5.0");
  if (ret != 0) {
    inno_log_error("set lidar key %s failed %d", client_set_key.c_str(), ret);
  }

  // set misorder_correct_enable to 1, in case of point packet disorder
  client_set_key = "LidarClient_StageClientRead/misorder_correct_enable";
  ret = inno_lidar_set_config_name_value(handle_, client_set_key.c_str(), "1");
  if (ret != 0) {
    inno_log_error("set lidar key %s failed %d", client_set_key.c_str(), ret);
  }
