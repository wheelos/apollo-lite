...
#include "modules/traffic_light/interface/stage.h"
 ...
 private:
  ...
  std::vector<VisualLight> DecodeRawYoloCandidates(
    ...
    for (const auto& candidate : context.raw_yolo_lights) {
      ...
      const PromptRegion* best_prompt = FindBestPrompt(context, roi);
      PromptSource prompt_src = PromptSource::FULL_FRAME;
      if (best_prompt != nullptr) {
        prompt_src = best_prompt->source;
      } else if (signal != nullptr) {
        prompt_src = PromptSource::MAP;
      }
      ...
      const float prompt_weight = best_prompt != nullptr ? best_prompt->weight : 0.0f;
      light.confidence = std::min(
          1.0f, 0.45f * candidate.objectness +
                    0.35f * candidate.semantic_confidence +
                    0.10f * signal_overlap +
                    0.10f * prompt_weight);
      light.signal_id = !candidate.signal_id.empty()
                            ? candidate.signal_id
                            : (signal != nullptr ? signal->signal_id
                                                 : (best_prompt != nullptr ? best_prompt->signal_id : ""));
      ...
      light.prompt_source = prompt_src;
      light.intended_movement =
          signal != nullptr
              ? signal->intended_movement
              : (best_prompt != nullptr && best_prompt->intended_movement != LaneIntent::UNKNOWN
                     ? best_prompt->intended_movement
      ...
      light.movement_mask =
          signal != nullptr
              ? NormalizeMovementMask(signal->movement_mask,
                                      signal->intended_movement,
                                      signal->shape_hint)
              : (best_prompt != nullptr && best_prompt->movement_mask != kMovementMaskNone
                     ? best_prompt->movement_mask
  ...
  const PromptRegion* FindBestPrompt(const PipelineContext& context,
                                     const Rect2f& roi) const {
    const PromptRegion* best = nullptr;
    float best_score = -1.0f;
    for (const auto& prompt : context.prompts) {
      if (prompt.source == PromptSource::FULL_FRAME) {
        continue;
      }
      const float iou = ComputeIou(prompt.roi_box, roi);
      const float score = iou * 0.7f + prompt.weight * 0.3f;
