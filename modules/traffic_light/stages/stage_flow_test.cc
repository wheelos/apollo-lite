#include "modules/traffic_light/stages/detector_stage.h"

#include <vector>

#include "gtest/gtest.h"
#include "modules/traffic_light/ports/collecting_result_writer_port.h"
#include "modules/traffic_light/ports/default_provider_ports.h"
#include "modules/traffic_light/stages/binder_stage.h"
#include "modules/traffic_light/stages/fusion_stage.h"
#include "modules/traffic_light/stages/heuristic_stage.h"
#include "modules/traffic_light/stages/prompter_stage.h"
#include "modules/traffic_light/stages/tracker_stage.h"
#include "modules/traffic_light/traffic_light_component.h"
...
TEST(TrafficLightStageTest, PrompterFeedbackAndDetectorPromptIntegration) {
  // Test the closed loop: T-1 track feedback -> Prompter -> Detector Prompt Association
  PipelineContext context;
  RuntimeState runtime;
  context.runtime_state = &runtime;
  context.primary_camera_name = "front_6mm";
  ...
  // 1. T-1 tracking feedback
  TrackedLight track;
  track.track_id = 101;
  track.confirmed = true;
  track.current_state.visual_light.camera_name = "front_6mm";
  track.current_state.visual_light.signal_id = "signal_temporal_1";
  track.current_state.visual_light.bbox = Rect2f{300.0f, 200.0f, 40.0f, 80.0f};
  runtime.tracked_memory.push_back(track);

  // Run Prompter
  PrompterStage prompter_stage{PrompterOptions()};
  ASSERT_TRUE(prompter_stage.Process(&context));
  EXPECT_FALSE(context.prompts.empty());

  // 2. Simulate raw neural candidate matching the prompted ROI
  YoloLightCandidate candidate;
  candidate.bbox = Rect2f{305.0f, 202.0f, 38.0f, 78.0f};
  candidate.objectness = 0.92f;
  candidate.semantic_confidence = 0.95f;
  candidate.class_id = 1;
  candidate.class_name = "red_circle";
  candidate.color = LightColor::RED;
  ...
  det_options.backend = DetectorBackendType::YOLO;
  DetectorStage detector_stage{det_options};
  ASSERT_TRUE(detector_stage.Process(&context));
  ASSERT_EQ(context.visual_lights.size(), 1u);
  // Verify that detector successfully associated the detection with the HISTORY prompt
  EXPECT_EQ(context.visual_lights.front().prompt_source, PromptSource::HISTORY);
  EXPECT_EQ(context.visual_lights.front().signal_id, "signal_temporal_1");
  EXPECT_EQ(context.visual_lights.front().color, LightColor::RED);
}

TEST(TrafficLightStageTest, PipelineEndToEndExecution) {
  // End-to-end multi-frame pipeline flow with full stages and result writer
  PerceptionPipeline pipeline;
  PrompterOptions prompter_opt;
  DetectorOptions detector_opt;
  detector_opt.backend = DetectorBackendType::YOLO;
  BinderOptions binder_opt;
  TrackerOptions tracker_opt;
  FusionOptions fusion_opt;

  pipeline.RegisterStage(std::make_shared<PrompterStage>(prompter_opt));
  pipeline.RegisterStage(std::make_shared<DetectorStage>(detector_opt));
  pipeline.RegisterStage(std::make_shared<BinderStage>(binder_opt));
  pipeline.RegisterStage(std::make_shared<TrackerStage>(tracker_opt));
  pipeline.RegisterStage(std::make_shared<FusionStage>(fusion_opt));
  ASSERT_TRUE(pipeline.InitAll());

  auto writer = std::make_shared<CollectingResultWriterPort>();
  RuntimeState runtime;

  // Frame 1: Detection occurs, tracked light established
  {
    PipelineContext ctx1;
    ctx1.frame_id = 1;
    ctx1.timestamp = 1000000000ULL;
    ctx1.runtime_state = &runtime;
    ctx1.primary_camera_name = "front_6mm";
    ctx1.nav_topology.ego_lane_intent = LaneIntent::STRAIGHT;

    CameraFrameState frame;
    frame.camera_name = "front_6mm";
    frame.image.rows = 1080;
    frame.image.cols = 1920;
    ctx1.camera_frames.push_back(frame);

    SignalCandidate sig;
    sig.signal_id = "signal_e2e_1";
    sig.projection_roi = Rect2f{400.0f, 200.0f, 30.0f, 60.0f};
    sig.confidence = 0.9f;
    sig.topology_confidence = 0.95f;
    sig.intended_movement = LaneIntent::STRAIGHT;
    sig.movement_mask = kMovementMaskStraight;
    ctx1.map_signals.push_back(sig);

    YoloLightCandidate yolo_cand;
    yolo_cand.bbox = Rect2f{402.0f, 201.0f, 28.0f, 58.0f};
    yolo_cand.objectness = 0.90f;
    yolo_cand.semantic_confidence = 0.92f;
    yolo_cand.class_id = 0;
    yolo_cand.color = LightColor::GREEN;
    yolo_cand.camera_name = "front_6mm";
    ctx1.raw_yolo_lights.push_back(yolo_cand);
    ctx1.status.neural_detector_ran = true;

    ASSERT_TRUE(pipeline.ProcessFrame(&ctx1));
    ASSERT_TRUE(writer->Write(ctx1, ctx1.primary_decision));

    EXPECT_EQ(writer->last_result().color, LightColor::GREEN);
    EXPECT_EQ(writer->last_result().signal_id, "signal_e2e_1");
    EXPECT_EQ(runtime.tracked_memory.size(), 1u);
  }

  // Frame 2: Uses temporal memory in Prompter, continues tracking smoothly
  {
    PipelineContext ctx2;
    ctx2.frame_id = 2;
    ctx2.timestamp = 1100000000ULL;
    ctx2.runtime_state = &runtime;
    ctx2.primary_camera_name = "front_6mm";
    ctx2.nav_topology.ego_lane_intent = LaneIntent::STRAIGHT;

    CameraFrameState frame;
    frame.camera_name = "front_6mm";
    frame.image.rows = 1080;
    frame.image.cols = 1920;
    ctx2.camera_frames.push_back(frame);
    ...
    sig.intended_movement = LaneIntent::STRAIGHT;
    sig.movement_mask = kMovementMaskStraight;
    ctx2.map_signals.push_back(sig);

    YoloLightCandidate yolo_cand;
    yolo_cand.bbox = Rect2f{401.0f, 200.0f, 29.0f, 59.0f};
    yolo_cand.objectness = 0.91f;
    yolo_cand.semantic_confidence = 0.94f;
    yolo_cand.class_id = 0;
    yolo_cand.color = LightColor::GREEN;
    yolo_cand.camera_name = "front_6mm";
    ctx2.raw_yolo_lights.push_back(yolo_cand);
    ctx2.status.neural_detector_ran = true;

    ASSERT_TRUE(pipeline.ProcessFrame(&ctx2));
    ASSERT_TRUE(writer->Write(ctx2, ctx2.primary_decision));

    EXPECT_EQ(writer->last_result().color, LightColor::GREEN);
    EXPECT_EQ(writer->last_result().signal_id, "signal_e2e_1");
    EXPECT_EQ(writer->history().size(), 2u);
  }
}
