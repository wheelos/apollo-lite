...
void Brakestatus511::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_overspd_env(overspd_env(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_brake_pedal_en_sts(brake_pedal_en_sts(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_brake_pedal_sts(brake_pedal_sts(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_brake_err(brake_err(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_emergency_btn_env(emergency_btn_env(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_front_bump_env(front_bump_env(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
      ->set_back_bump_env(back_bump_env(bytes, length));
  MutableChassisExtension<::apollo::canbus::Ch>(chassis)
      ->mutable_brake_status__511()
