...
class Errorclearcommand441 : public ::apollo::drivers::canbus::ProtocolData<
 ...
 public:
  ...
  Errorclearcommand441* set_error_clear_cmd(int error_clear_cmd);

 private:

  // config detail: {'bit': 7, 'is_signed_var': False, 'len': 8, 'name': 'Error_Clear_Cmd', 'offset': 0.0, 'order': 'motorola', 'physical_range': '[0|255]', 'physical_unit': '', 'precision': 1.0, 'type': 'int'}
  void set_p_error_clear_cmd(uint8_t* data, int error_clear_cmd);

  int error_clear_cmd(const std::uint8_t* bytes, const int32_t length) const;

 private:
  int error_clear_cmd_;
};
