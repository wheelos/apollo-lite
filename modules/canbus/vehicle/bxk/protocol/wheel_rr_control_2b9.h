...
class Wheelrrcontrol2b9 : public ::apollo::drivers::canbus::ProtocolData<
 ...
 public:
  ...
  Wheelrrcontrol2b9* set_speed_rr_ctrl(int speed_rr_ctrl);
  ...
  Wheelrrcontrol2b9* set_runningstate_rr(Wheel_rr_control_2b9::Runningstate_rrType runningstate_rr);
  ...
  Wheelrrcontrol2b9* set_directionstate_rr(Wheel_rr_control_2b9::Directionstate_rrType directionstate_rr);

 private:

  // config detail: {'bit': 0, 'is_signed_var': True, 'len': 32, 'name': 'Speed_rr_Ctrl', 'offset': 0.0, 'order': 'intel', 'physical_range': '[0|1000]', 'physical_unit': 'RPM', 'precision': 1.0, 'type': 'int'}
  void set_p_speed_rr_ctrl(uint8_t* data, int speed_rr_ctrl);

  // config detail: {'bit': 32, 'enum': {0: 'RUNNINGSTATE_RR_STOPPED', 2: 'RUNNINGSTATE_RR_RUNNING'}, 'is_signed_var': False, 'len': 8, 'name': 'RunningState_rr', 'offset': 0.0, 'order': 'intel', 'physical_range': '[0|2]', 'physical_unit': '', 'precision': 1.0, 'type': 'enum'}
  void set_p_runningstate_rr(uint8_t* data, Wheel_rr_control_2b9::Runningstate_rrType runningstate_rr);

  // config detail: {'bit': 40, 'enum': {0: 'DIRECTIONSTATE_RR_REVERSE', 1: 'DIRECTIONSTATE_RR_FORWARD'}, 'is_signed_var': False, 'len': 8, 'name': 'DirectionState_rr', 'offset': 0.0, 'order': 'intel', 'physical_range': '[0|1]', 'physical_unit': '', 'precision': 1.0, 'type': 'enum'}
  void set_p_directionstate_rr(uint8_t* data, Wheel_rr_control_2b9::Directionstate_rrType directionstate_rr);
 ...
 private:
  int speed_rr_ctrl_;
  Wheel_rr_control_2b9::Runningstate_rrType runningstate_rr_;
  Wheel_rr_control_2b9::Directionstate_rrType directionstate_rr_;
};
