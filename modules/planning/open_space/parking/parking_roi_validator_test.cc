...
TEST(ParkingRoiValidatorTest, RejectVehicleFootprintOutsideGeometryOnly) {
  OpenSpaceRoiDeciderConfig config;
  ParkingRoiValidator validator(config);

  const ParkingRoiGeometry geometry = MakeRectangularGeometry();
  const auto result = validator.ValidateGeometryOnly(
      geometry, Vec2d(0.3, 3.0), 0.0, MakeVehicleParam());
  EXPECT_FALSE(result.valid);
  EXPECT_FALSE(result.vehicle_inside);
  EXPECT_EQ(result.reason, "ego vehicle box outside parking roi");
}
...
TEST(ParkingRoiValidatorTest, RejectGoalOutsideRoi) {
  ...
  EXPECT_FALSE(result.goal_inside_roi);
  EXPECT_EQ(result.reason, "goal vehicle box outside parking roi");
}
...
TEST(ParkingRoiValidatorTest, AcceptGoalOutsideSlotEnvelopeInsideWideRoi) {
  OpenSpaceRoiDeciderConfig config;
  ParkingRoiValidator validator(config);
  ...
  EXPECT_TRUE(result.valid);
  EXPECT_TRUE(result.goal_inside);
  EXPECT_TRUE(result.goal_inside_roi);
  EXPECT_FALSE(result.goal_inside_envelope);
  EXPECT_TRUE(result.reason.empty());
  EXPECT_GT(result.goal_clearance, 0.0);
}

TEST(ParkingRoiValidatorTest,
     RejectGoalOutsideParkingRoiEvenWhenEnvelopeCanContainGoal) {
  OpenSpaceRoiDeciderConfig config;
  ParkingRoiValidator validator(config);
  ParkingPoseSelector selector(config);

  const auto slot = MakePerpendicularRightSlot();
  auto geometry = MakeWideGeometryForSlot(slot, 6.0);
  geometry.union_polygon = {Vec2d(-0.7, -12.0), Vec2d(0.7, -12.0),
                            Vec2d(0.7, 1.0), Vec2d(-0.7, 1.0)};
  geometry.xy_boundary = {-0.7, 0.7, -12.0, 1.0};
  PopulateBoundarySegments(&geometry);

  const auto selection = selector.Select(slot, geometry, Make4p0VehicleParam(),
                                         Vec2d(0.0, 12.0), -M_PI_2);
  ASSERT_TRUE(selection.has_feasible_candidate());

  const auto result = validator.Validate(geometry, Vec2d(0.0, 0.0),
                                         selection.selected().end_pose,
                                         Make4p0VehicleParam());
  EXPECT_FALSE(result.valid);
  EXPECT_FALSE(result.goal_inside);
  EXPECT_FALSE(result.goal_inside_roi);
  EXPECT_TRUE(result.goal_inside_envelope);
  EXPECT_EQ(result.reason, "goal vehicle box outside parking roi");
}
...
TEST(ParkingRoiValidatorTest,
  ...
  const std::vector<double> goal_pose = {-1.16894, -3.02872, 1.20246, 0.0};
  const auto result = validator.Validate(geometry, Vec2d(0.0, 0.0),
                                         goal_pose, Make4p0VehicleParam());
  EXPECT_TRUE(result.valid);
  EXPECT_TRUE(result.goal_inside);
  EXPECT_GT(result.goal_clearance, 0.0);
}
