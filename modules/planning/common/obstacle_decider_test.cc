...
class ObstacleDeciderTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    BuildStraightReferenceLine(200.0);
  }

  void BuildStraightReferenceLine(double length) {
    std::vector<apollo::hdmap::MapPathPoint> points;
    for (double s = 0.0; s <= length; s += 1.0) {
      points.emplace_back(apollo::common::math::Vec2d(s, 0.0), s);
    }

    apollo::hdmap::Path map_path(points);
    reference_line_ = std::make_unique<ReferenceLine>(map_path);
  }
