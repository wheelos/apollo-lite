...
ObjectDecisionType Obstacle::MergeLongitudinalDecision(
  ...
  const auto lhs_val =
      s_longitudinal_decision_safety_sorter_.at(lhs.object_tag_case());
  const auto rhs_val =
      s_longitudinal_decision_safety_sorter_.at(rhs.object_tag_case());
  if (lhs_val < rhs_val) {
    return rhs;
...
ObjectDecisionType Obstacle::MergeLateralDecision(
  ...
  const auto lhs_val =
      s_lateral_decision_safety_sorter_.at(lhs.object_tag_case());
  const auto rhs_val =
      s_lateral_decision_safety_sorter_.at(rhs.object_tag_case());
  if (lhs_val < rhs_val) {
    return rhs;
  } else if (lhs_val > rhs_val) {
    return lhs;
