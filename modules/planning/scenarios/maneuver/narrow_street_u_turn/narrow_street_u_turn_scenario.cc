...
class NarrowStreetUTurnStage final : public Stage {
 public:
  NarrowStreetUTurnStage(const ScenarioConfig::StageConfig& config,
  ...
  StageStatus Process(const common::TrajectoryPoint& planning_init_point,
    ...
    if (!plan_ok) {
      AERROR << "NarrowStreetUTurnStage failed to produce a plan.";
...
std::unique_ptr<Stage> NarrowStreetUTurnScenario::CreateStage(
    ...
    const std::shared_ptr<DependencyInjector>& injector) {
  if (stage_config.stage_type() == StageType::NARROW_STREET_U_TURN_DEFAULT_STAGE) {
    return std::make_unique<NarrowStreetUTurnStage>(stage_config, injector);
  }
  return nullptr;
}
