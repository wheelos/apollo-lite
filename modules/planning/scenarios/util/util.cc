...
PullOverStatus CheckADCPullOver(
  ...
  const common::math::Vec2d adc_position = {reference_state.x(),
                                            reference_state.y()};
  ...
  const bool position_check = CheckPullOverPositionBySL(
      ...
      reference_state.heading(), target_position,
...
bool CheckADCReadyToCruise(
  ...
  common::math::Vec2d adc_position = {reference_state.x(), reference_state.y()};
  const double adc_heading = reference_state.heading();
  ...
  return ((operating_state.travel_direction() == TRAVEL_DIRECTION_FORWARD ||
           std::fabs(reference_state.linear_velocity()) <
               kMinSpeed) &&
