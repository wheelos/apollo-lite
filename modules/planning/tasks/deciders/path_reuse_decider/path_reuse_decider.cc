...
bool PathReuseDecider::IsIgnoredBlockingObstacle(
  ...
  // vehicle speed
  double adc_speed = reference_line_info->reference_state().linear_velocity();
  double final_s_buffer = std::max(kSDistBuffer, kTimeBuffer * adc_speed);
  // current vehicle s position
  common::SLPoint adc_position_sl;
  GetADCSLPoint(reference_line, reference_line_info->reference_state(),
                &adc_position_sl);
...
bool PathReuseDecider::TrimHistoryPath(
  ...
  GetADCSLPoint(reference_line, frame->reference_state(), &adc_position_sl);
