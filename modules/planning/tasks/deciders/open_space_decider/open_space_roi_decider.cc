...
void PopulateParkingDebug(
  ...
  if (pose_selection.has_feasible_candidate()) {
    ...
    open_space_debug->mutable_end_point()->CopyFrom(MakeDebugTrajectoryPoint(
        pose_selection.selected().end_pose[0],
...
void SampleParkingRoadBoundariesDense(
  ...
  const double end_s = std::max(
      start_s,
      std::min(path_end_s,
               center_line_s + config.roi_longitudinal_range_end()));
  const double step = 0.2;

  for (double sample_s = start_s; sample_s <= end_s + common::math::kMathEpsilon;
       sample_s += step) {
    const double clamped_s =
        std::max(start_s, std::min(sample_s, end_s));
...
std::vector<Vec2d> BuildSlotSideConnectorBoundary(
    ...
    const std::vector<double> &boundary_s, const std::size_t connection_start_index,
    const std::size_t connection_end_index,
    const double opening_start_s, const double opening_start_l,
    const double opening_end_s, const double opening_end_l,
...
void UpdateParkingDebug(
    ...
    const parking::ParkingPoseSelection &pose_selection,
    const parking::ParkingRoiValidationResult *validation_result) {
  auto *parking_debug = frame->mutable_open_space_info()->mutable_parking_debug();
...
void ApplyParkingRoiToOpenSpaceInfo(
  ...
  auto *roi_polygon = mutable_open_space_info->mutable_roi_parking_boundary_polygon();
...
bool LoadParkingRoiGeometryFromOpenSpaceInfo(
  ...
  if (geometry->union_polygon.size() < 3U || geometry->xy_boundary.size() < 4U) {
    if (error != nullptr) {
...
bool CheckDistanceToParkingSpot(
  ...
  if (!apollo::planning::util::GetParkingSpotCenterFromRouting(
    ...
    parking_spot_center =
        apollo::planning::util::GetParkingSpotCenterFromMap(target_parking_spot);
  }
...
bool ResolveTargetParkingLane(
  ...
  if (ptr_last_frame == nullptr) {
    if (error != nullptr) {
      *error = "last frame unavailable when resolving target parking lane";
    }
    return false;
  }
  ...
  std::size_t nearest_lane_index = 0U;
...
bool BuildNearbyPath(const hdmap::HDMap *hdmap,
  ...
  LaneSegment nearest_lane_segment(
      nearest_lane, nearest_lane->accumulate_s().front(),
  ...
  for (int i = 0; i < next_lanes_num; ++i) {
    ...
    for (int j = 0; j < succeed_lanes_num; ++j) {
      auto succeed_lane =
          hdmap->GetLaneById(next_lane->lane().successor_id(j));
...
bool BuildParkingRoiGeometryFromBoundaries(
  ...
  const auto &slot_side_outer_boundary = slot_on_left ? left_boundary : right_boundary;
...
void OpenSpaceRoiDecider::GetRoadBoundaryFromMap(
  ...
  while (check_point_s <= end_s + common::math::kMathEpsilon) {
    ...
    check_point_s =
        std::min(end_s, check_point_s + config_.open_space_roi_decider_config()
...
bool OpenSpaceRoiDecider::GetParkingBoundary(
  ...
  if (!GetTargetParkingSpotById(hdmap_, parking_spot_id, &target_parking_spot)) {
    if (error != nullptr) {
  ...
  *frame->mutable_open_space_info()->mutable_target_parking_spot() =
  ...
  if (!CheckDistanceToParkingSpot(config_.open_space_roi_decider_config(), frame,
                                  vehicle_state_, target_parking_spot)) {
    if (error != nullptr) {
      *error = "target parking spot is farther than parking_start_range";
    }
    return false;
  }
  ...
  if (!provider.BuildFromMap(target_parking_spot, nearby_path, &parking_slot,
                             error)) {
    return false;
  }
  ...
  if (FLAGS_use_road_boundary_from_map) {
    ...
    SampleParkingRoadBoundariesDense(config_.open_space_roi_decider_config(),
                                     nearby_path, center_line_s, origin_point,
                                     origin_heading, &left_lane_boundary,
                                     &right_lane_boundary, &center_lane_s_left,
                                     &center_lane_s_right,
                                     &left_lane_road_width,
                                     &right_lane_road_width);
  }
  ...
  if (!BuildParkingRoiGeometryFromBoundaries(
          ...
          &roi_geometry, roi_parking_boundary, error)) {
  ...
  if (roi_config.has_candidate_warm_start_config()) {
    ...
    start_pose_buffer =
        std::clamp(0.5 * roi_config.candidate_warm_start_config()
                              .xy_grid_resolution(),
                   0.05, 0.15);
  }
  ...
  if (!parking::ExpandParkingRoiToIncludeVehicleFootprint(
          ...
          start_escape_distance, &roi_geometry, error)) {
    parking::ParkingPoseSelection empty_selection;
    parking::ParkingRoiValidationResult invalid_result;
  ...
  *roi_parking_boundary = roi_geometry.boundary_segments;
  ...
  parking::ParkingRoiValidator roi_validator(
      config_.open_space_roi_decider_config());
  const auto roi_geometry_validation =
      roi_validator.ValidateGeometryOnly(roi_geometry, vehicle_xy);
  if (!roi_geometry_validation.valid) {
    parking::ParkingPoseSelection empty_selection;
    UpdateParkingDebug(frame, roi_geometry, empty_selection,
                       &roi_geometry_validation);
    if (error != nullptr) {
      *error = "parking roi geometry validation failed: " +
  ...
  parking::ParkingPoseSelector pose_selector(
      config_.open_space_roi_decider_config());
  const auto pose_selection = pose_selector.Select(
      normalized_slot, roi_geometry, vehicle_params_, vehicle_xy,
      vehicle_heading);
  if (!pose_selection.has_feasible_candidate()) {
...
bool OpenSpaceRoiDecider::ValidateROIOnVertices(Frame *const frame) {
  ...
  std::string error;
  std::fprintf(stderr, "[roi-validate] begin\n");
  ...
  std::fprintf(stderr,
               "[roi-validate] geometry union=%zu slot=%zu bridge=%zu xy=%zu "
               "area=%.6f aisle=%.6f\n",
               geometry.union_polygon.size(), geometry.slot_polygon.size(),
               geometry.bridge_polygon.size(), geometry.xy_boundary.size(),
               geometry.area, geometry.aisle_width);

  const auto &end_pose = frame->open_space_info().open_space_end_pose();
  if (end_pose.size() < 2U) {
    error = "parking end pose unavailable for roi validation";
    AERROR << error;
    return false;
  }
  std::fprintf(stderr,
               "[roi-validate] end_pose size=%zu x=%.6f y=%.6f phi=%.6f\n",
               end_pose.size(), end_pose[0], end_pose[1],
               end_pose.size() >= 3U ? end_pose[2] : 0.0);

  parking::ParkingRoiValidator validator(
      config_.open_space_roi_decider_config());
  std::fprintf(stderr, "[roi-validate] before validator\n");
  const parking::ParkingRoiValidationResult validation_result =
      validator.Validate(geometry, Vec2d(0.0, 0.0), end_pose, vehicle_params_);
  std::fprintf(stderr,
               "[roi-validate] after validator valid=%d reason=%s "
               "goal_clearance=%.6f\n",
               validation_result.valid, validation_result.reason.c_str(),
               validation_result.goal_clearance);
  std::fprintf(stderr, "[roi-validate] before debug update\n");
  UpdateParkingValidationDebug(frame, validation_result);
  std::fprintf(stderr, "[roi-validate] after debug update\n");
