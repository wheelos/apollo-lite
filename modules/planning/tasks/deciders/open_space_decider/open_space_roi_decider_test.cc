...
TEST_F(OpenSpaceRoiDeciderTest, ValidateParkingRoiAcceptsWorkflowOutput) {
  ...
  PopulateParkingRoi(&frame,
                     {Vec2d(-2.0, -3.0), Vec2d(10.0, -3.0), Vec2d(10.0, 3.0),
                      Vec2d(-2.0, 3.0)},
                     {-2.0, 10.0, -3.0, 3.0}, {5.0, 0.0, 0.0, 0.0});
...
TEST_F(OpenSpaceRoiDeciderTest,
  ...
  PopulateParkingRoi(&frame,
                     {Vec2d(-2.0, -3.0), Vec2d(10.0, -3.0), Vec2d(10.0, 3.0),
                      Vec2d(-2.0, 3.0)},
                     {-2.0, 10.0, -3.0, 3.0}, {5.95, 0.0, 0.0, 0.0});
  ...
  EXPECT_TRUE(parking_roi.is_valid());
  EXPECT_EQ(parking_roi.invalid_reason(), "");
  EXPECT_DOUBLE_EQ(parking_roi.goal_clearance(), 0.0);
}
...
TEST_F(OpenSpaceRoiDeciderTest,
       ValidateParkingRoiUsesCurrentEgoPoseInsteadOfOriginPoint) {
  VehicleState vehicle_state;
  vehicle_state.set_x(15.0);
  vehicle_state.set_y(20.0);
  vehicle_state.set_heading(0.0);
  Frame frame(0, LocalView{}, TrajectoryPoint{}, vehicle_state, nullptr);
  ...
  PopulateParkingRoi(
      &frame,
      {Vec2d(3.0, -3.0), Vec2d(11.0, -3.0), Vec2d(11.0, 3.0), Vec2d(3.0, 3.0)},
      {3.0, 11.0, -3.0, 3.0}, {7.0, 0.0, 0.0, 0.0});

  EXPECT_TRUE(decider_->ValidateROIOnVertices(&frame));

  const auto& parking_roi = frame.open_space_info()
                                .debug_instance()
                                .planning_data()
                                .open_space()
                                .parking_roi();
...
TEST_F(OpenSpaceRoiDeciderTest, PullOverEndPoseClearsPreviousValues) {
  Frame frame = MakeFrame();
  *frame.mutable_open_space_info()->mutable_open_space_end_pose() = {9.0, 8.0,
                                                                     7.0, 6.0};
  frame.mutable_open_space_info()->set_origin_heading(0.0);
  frame.mutable_open_space_info()->mutable_origin_point()->set_x(10.0);
  frame.mutable_open_space_info()->mutable_origin_point()->set_y(20.0);
